...
def buildArray(dim, filename):
    f = open(filename, "r")
    matrixA = [[int(f.readline().strip('\n')) for i in range(dim)] for j in range(dim)]
    matrixB = [[int(f.readline().strip('\n')) for i in range(dim)] for j in range(dim)]
...
if __name__ == "__main__":
    A = []
    B = []
    A, B = buildArray(int(sys.argv[2]), sys.argv[3])
    if sys.argv[1] == "0":
        C = helper(A, B)
        printDiagonal(C)
